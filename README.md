...
### 💡 이슬기
```
배운 점으로는 첫째로, Spring만 사용해서 해당 환경에 익숙해져 있었는데 오랜만에 Servlet을 활용해서 기본 환경설정을 배울 수 있었다.
둘째로, 이렇게 단기 프로젝트를 할 때면 ERD과 database를 처음부터 잘 짜는 게 정말 중요하다,, 라는 점이다.
아쉬운 점으로는 단기 프로젝트라서 기능 구현이 가장 우선이기 때문에 상대적으로 최적화나 Refactoring 할 시간이
부족했다는 점이다.
```

### 💡 김민성
```
이번 프로젝트를 통해 maven 대신 gradle 이라는 빌드도구를 처음 접해보게 되었는데 플러그인 스크립트의 가독성이 maven 스크립트인 pom.xml 보다 뛰어났다.
또한 servlet 의 동작원리에 대해 확실하게 알 수 있었고 기능을 구현할 때 사용자 입장에서 편의성을 고려해볼 수 있는 계기가 되었다. 
아쉬운 점으로는 초기 설계 단계와 개발 환경 통일이 매우 매우 중요하다는 것을 느꼈다. 
개발 환경을 통일하지 않은 채로 개발을 진행하다보니 깃허브를 이용할때마다 서로 충돌이 많이 나서 개발 시간보다 충돌 오류 해결에 시간을 더 쏟았던 것 같다.  
또한 초기 설계 단계에서 테이블명, 데이터들의 타입, 컬럼명을 명확하게 정하지 않고 개발 도중 수정하다보니 혼동되는 부분이 조금 있었다.
```
